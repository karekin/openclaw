...
class TushareAPI:
    ...
    def __init__(self, token: Optional[str] = None, http_url: Optional[str] = None):
        ...
        Args:
            ...
            http_url: Tushare 反代地址，默认从环境变量 TUSHARE_HTTP_URL 读取
        """
        ...
        # 兼容反代接入和部分代理场景下对私有属性的依赖。
        setattr(self.pro, '_DataApi__token', self.token)
        if self.http_url:
            setattr(self.pro, '_DataApi__http_url', self.http_url.rstrip('/'))
            logger.info("Tushare API 使用反代地址: %s", self.http_url)
